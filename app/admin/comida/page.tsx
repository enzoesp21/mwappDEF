...
const PCFG = {
  ...
  celiaco: { label: 'Celiaco', color: 'text-amber-600', bg: 'bg-amber-50' },
        ...
        return (
          <div key={day.date} className="bg-brand-card border border-brand-border rounded-2xl p-4 space-y-4">
            ...
            {day.meals.map((meal) => {
              ...
              return (
                <div key={meal}>
                  <p className="text-xs font-semibold text-brand-muted uppercase tracking-wide mb-2 capitalize">{meal}</p>
                  <div className="flex flex-wrap gap-2 mb-3">
                    {Object.entries(PCFG).map(([key, cfg]) => {
                      const count = mealSignups.filter((s) => s.preference === key).length
                      if (count === 0) return null
                      return (
                        <span key={key} className={cn('text-xs font-medium px-2 py-0.5 rounded-full', cfg.bg, cfg.color)}>
                          {cfg.label}: {count}
                        </span>
                      )
                    })}
                    ...
                    {mealSignups.map((s, i) => {
                      ...
                      return (
                        <div key={i} className="flex items-center gap-3 py-2">
                          ...
                          <span className={cn('text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full', cfg?.bg ?? 'bg-brand-accent/10', cfg?.color ?? 'text-brand-accent')}>
